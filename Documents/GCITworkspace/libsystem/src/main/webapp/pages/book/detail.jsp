...
%>
	...
	<h2>Book Details</h2>
		...
		<div class="row">
			...
			<div class="col-md-4">
				<form class="form-signin" action="/libsystem/pages/book/saveBook">
					<input
						type="hidden" name="bookId" value="<%=id%>">
					<h5 class="form-signin-heading">Update</h5>
					<input type="text" id="inputBook" name="bookTitle" class="form-control"
						value="<%=bookTitle%>" required autofocus> <br>
						<h5> Publisher
					<select  name="publisherId" class="selectpicker" data-live-search="true" >
					<%	for(Publisher pub: publishers){   
					%>
						<% if(pub.getPublisherId() == publisher.getPublisherId()) { %>
							<option value="<%=publisherId%>" selected><%=publisherName%></option>
						<% } else { %>
					...
					<select name="authorId" class="selectpicker" data-live-search="true" multiple>
					...
					<select name="genre_id" class="selectpicker" data-live-search="true" multiple>
					<%
